...
name: capture
description: >
  Capture insights and add, replace, edit, or remove attached files (images, PDFs,
  slides, or any file type) for named entities, including custom types, deals, or a
...
- Attachments require valid hexadecimal UUIDs, actual persisted files, and the
...
Capture only a concrete assertion about a named asset, or an explicitly requested
attachment-only note as defined in the attachment contract:
...
Persist only approved observation records, approved managed attachment bytes,
and explicitly requested artifact
