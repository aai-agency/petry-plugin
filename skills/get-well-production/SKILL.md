...
`assets/preview.html` renders the **actual `@aai-agency/og-components` `LineChart`**
(React + uPlot bundled inline) inside a card shell. It is fully self-contained
and offline. **Do not hand-edit it** — it is generated by `build/build.mjs`
(see `build/README.md`); only fill the two placeholders.
...
3. Build a `CHART_DATA` JSON object (contract at the top of `preview.html`): well
   ...
   the `activity` from step 2. The real `LineChart` plots oil/gas/water (gas on a
   right axis); the captured `activity` is listed in the card's **Activity panel**.
   If the user didn't give production numbers, say you're using representative
   sample values and make that visible — never present synthesized numbers as if
   they were the user's real data.
