...
light theme, fully self-contained and offline. **Do not hand-edit it** — it is
generated by `build/build.mjs` (see `build/README.md`); only fill the two
placeholders.
...
3. Build a `CHART_DATA` JSON object (contract at the top of `preview.html`): well
   ...
   The real `DeclineCurve` plots the oil actuals + a forecast; each captured
