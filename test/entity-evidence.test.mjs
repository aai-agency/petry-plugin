...
const dependencies = {schema_version: 1, consumes_insights: true, includes_undated: true, loaded_world_window: {from: null, to: null}, insight_fields_used: ['petry.attachments']};
...
test('rollup oracle rejects duplicated multi-subject notes, ancestor leakage and lost evidence', () => {
  ...
  const artifact = {petry_dependencies: {...dependencies, loaded_asset_refs: scopeFor(graph, 'asset:deal'), entity_scope: {mode: 'descendants', root_asset_refs: ['asset:deal']}}, activity: records.slice(0, 2)};
  ...
  const child = {petry_dependencies: {...dependencies, loaded_asset_refs: ['asset:well'], entity_scope: {mode: 'direct', root_asset_refs: ['asset:well']}}, activity: [records[0]]};
